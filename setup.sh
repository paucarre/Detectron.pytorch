...
#
# INSTALL 'detectron' ENVIRONMENT WITH ALL DEPENDENCIES
#
ENVS=$(conda env list | awk '{print $1}' )
if [[ $ENVS = *detectron* ]]; then
   echo -e "Anaconda 'detectron' environment detected."
   conda activate detectron
else
   #
   # INSTALL DEPENDENCIES
   #
   echo -e "Anaconda 'detectron' environment not detected.\nInstalling anaconda environment 'detectron' together with all the required dependencies..."
   conda update --all -y
   ...
   conda deactivate
   conda activate detectron
   echo "Run 'conda activate detectron' to enable python virtual environment to run the scripts."
fi;
